      ...
      {filteredJobs.length > 0 ? (
        <div className='flex gap-5 flex-wrap'>
          {filteredJobs.map((job, index) => (
            <Link
              href={`${ROUTES.CLIENT_JOB_POSTINGS}/${slugify(job.id)}`}
              key={index}
              className='relative w-[400px] h-[400px] rounded-lg shadow-sm overflow-hidden'
            >
              {/* Job Image */}
        ...
        <div className='flex flex-col justify-center items-center my-72 gap-4'>
          ...
          <h1 className='text-gray-900 text-Text-lg font-semibold'>No posts yet</h1>

          <p className='text-center text-gray-900 opacity-70'>
            Showcase your projects and attract clients who need your expertise. Whether you&apos;re a writer, <br />{' '}
            designer, developer, or have any other skill to offer, this is your platform to shine.
          </p>
        </div>
      )}
    </div>
